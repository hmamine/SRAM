...
print (" These R scripts display the ternary plot indicated in Fig 3B and corresponding venn diagram in 2D")

Palette.phylum <-c("#4e8e8e","#8e8e4e","#8e4e8e","#000000")
...
# upload and prepare phyloseq objects***
	...
	DT.taxa$ColPhylum <- ifelse(!DT.taxa$Phylum %in% ColPhylum, "Other", ifelse(DT.taxa$Phylum == "p__Ascomycota", "Ascomycota", ifelse(DT.taxa$Phylum == "p__Basidiomycota", "Basidiomycota","Mortierellomycota")))
...
###computing differentially ASVs === Manresa Vs Marquis	
	...
	physeq.i=subset_samples( physeqF2, Cultivar %in% c("Manresa", "Marquis"))
...
###computing differentially ASVs === Manresa Vs SweetAnne
	...
	physeq.i=subset_samples( physeqF2, Cultivar %in% c("Manresa", "SweetAnne"))
...
###computing differentially ASVs === Marquis Vs SweetAnne
	...
	physeq.i=subset_samples( physeqF2, Cultivar %in% c("Marquis", "SweetAnne"))
...
# transformt counts to rel. abundance
	physeq.ra = transform_sample_counts(physeqF2, function(x) x/sum(x))
...
#	pdf("ternary_rhizo_fungi.pdf", useDingbats=FALSE)
	print(p1)
#	dev.off()
...
#	write.table(DT.uniq, "TAXA_root_Fun.txt", sep="\t")
