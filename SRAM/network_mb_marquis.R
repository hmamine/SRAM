...
# 	save subnetwork metrics	
	...
	write.table(DT.m, "marquis_metrics_full.txt", sep="\t")
	write.table(DT_sub, "marquis_metrics_sub.txt", sep="\t")
	write.table(as_data_frame(graph.mb), "marquis_network_full.txt", sep="\t")
	write.table(as_data_frame(selecG), "marquis_network_sub", sep="\t")
	write.table(DT_m3, "marquis_sub_relative.txt", sep="\t")

#	write.table(otu_table(NewOTU), "marquis_Species_OTU_tab.txt", sep="\t")
