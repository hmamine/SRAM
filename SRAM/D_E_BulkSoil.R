...
print (" This R scripts reproduce figures displayed in Supp Fig 1C and 1D"); 
...
# upload and prepare phyloseq objects***
	...
	physeq.i=subset_samples( physeq1, SampleType == "Bulk" )
		
	DT.taxa=data.table( as(tax_table(physeq.i),"matrix"), keep.rownames=T, key="rn" )

###computing differentially abundant bacterial ASVs
		...
		DT1$FC <- ifelse ( DT1$logFC > 0, "Enr", "Dep")
		DT1$Col <- ifelse (DT1$adj.P.Val < alpha & abs( DT1$logFC ) > folch, DT1$ColPhylum,"NS" )

	DT2<-DT1
	p1<-Manhat( aes( y=-log2(adj.P.Val), x=ColPhylum, color = Col, shape=FC ))
	p2<-Volcan( aes( y=-log2(adj.P.Val), x=logFC, color = Col ))
...
###upload fungal data 
	...
	physeq.i=subset_samples( physeqF1, SampleType == "Bulk" )
...
###computing differentially abundant fungal ASVs
	...
	PhylumFun <-c("#8e4e8e","#8e8e4e","#88888a","#4e8e8e")
	DT2<-DT3
...
#	pdf("Supp_fig_1.pdf",useDingbats=FALSE)
	gridExtra::grid.arrange(p2,p4, ncol=2, nrow=2)
#	dev.off()
